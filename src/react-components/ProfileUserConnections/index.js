...
export default class ProfileUserConnections extends React.Component {
  ...
  renderUserConnections() {
    ...
    */
    ...
    let listItems;
    if (this.state.showingFollowers) {
      listItems = followers.map((f, index) => {
        return (<SmallProfileBar key={index} isFollower={ true } name={ f.name } username={ f.username } imgSrc={ f.profilePicSrc } />);
      });
    } else {
      listItems = following.map((f, index) => {
        return(<SmallProfileBar key={index} isFollower={ false } name={ f.name } username={ f.username } imgSrc={ f.profilePicSrc } />);
      });
    }
